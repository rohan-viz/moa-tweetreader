...
public class WriteStreamToARFFFile extends MainTask {
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
        ...
        if (destFile != null) {
            try {
			   //Instance[] buffer = new Instance[this.maxInstancesOption.getValue()];
               int numWritten = 0;
			   while ((numWritten < this.maxInstancesOption.getValue())
                        && stream.hasMoreInstances()) {
                    //buffer[numWritten] = stream.nextInstance();
                    stream.getHeader().add(stream.nextInstance());
                    numWritten++;
                }
                ...
                while ((numWritten < this.maxInstancesOption.getValue())
                        && stream.hasMoreInstances()) {
                    //w.write(buffer[numWritten].toString());
                    w.write(stream.getHeader().get(numWritten).toString());
                    w.write("\n");
                    numWritten++;
                }
                w.close();
                if (stream instanceof TweetReader) {
                     ((TweetReader) stream).shutdown();
                }
            } catch (Exception ex) {
